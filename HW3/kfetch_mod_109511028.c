...
static void get_system_info(unsigned int info_mask, char *output_buffer)
{
    ...
    char hostname[NORMAL_BUF_SIZE], releaseInfo[NORMAL_BUF_SIZE];
    int hostname_length = snprintf(hostname, NORMAL_BUF_SIZE, utsname_info->nodename);
    ...
    if (mask_info & KFETCH_RELEASE) {
        ...
        snprintf(releaseInfo, NORMAL_BUF_SIZE, "Kernel:   %s\n", utsname_info->release);
    ...
    c = &cpu_data(cpu);
    if(c->x86_model_id[0]) {
	    strcpy(CPUModel, c->x86_model_id);
    }
    else{
	    strcpy(CPUModel, c->x86);
    }
    ...
    char CPUInfo[NORMAL_BUF_SIZE] = "";
    if (mask_info & KFETCH_NUM_CPUS) {
        pr_info("Number of CPUs information\n");
        snprintf(CPUInfo, NORMAL_BUF_SIZE, "CPUs:     %d / %d\n", num_online_cpus(), num_possible_cpus());
        strcat(output_buffer, logo[current_logo_row++]);
    ...
    char MemInfo[NORMAL_BUF_SIZE] = "";
    if (mask_info & KFETCH_MEM) {
        pr_info("Memory information\n");
        snprintf(MemInfo, NORMAL_BUF_SIZE, "Mem:      %lu MB / %lu MB\n", free_mem_mb, total_mem_mb);
        strcat(output_buffer, logo[current_logo_row++]);
    ...
    char ProcInfo[NORMAL_BUF_SIZE] = "";
    if (mask_info & KFETCH_NUM_PROCS) {
        pr_info("Number of Processes information\n");
        snprintf(ProcInfo, NORMAL_BUF_SIZE, "Procs:    %d\n", num_processes);
        strcat(output_buffer, logo[current_logo_row++]);
    ...
    char UptimeInfo[NORMAL_BUF_SIZE] = "";
    if (mask_info & KFETCH_UPTIME) {
        pr_info("Uptime information\n");
        snprintf(UptimeInfo, NORMAL_BUF_SIZE, "Uptime:   %lu mins\n", uptime_minutes);
        strcat(output_buffer, logo[current_logo_row++]);
